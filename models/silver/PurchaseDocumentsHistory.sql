WITH PurchaseDocumentsHistory AS (
SELECT
...
FROM
  {{ source("source_db", "ekbe") }} AS ekbe
LEFT JOIN {{ ref("currency_decimal") }} AS currency_decimal
  ON ekbe.WAERS = currency_decimal.CURRKEY
LEFT JOIN {{ ref("calendar_date_dim") }} AS CalendarDateDimension_BUDAT
  ON CalendarDateDimension_BUDAT.Date = ekbe.BUDAT
LEFT JOIN {{ ref("calendar_date_dim") }} AS CalendarDateDimension_BLDAT
...
SELECT * FROM PurchaseDocumentsHistory